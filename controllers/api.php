...
class Api extends Oauth_Controller
{
	...
	{
		if ($user = $this->social_auth->get_user('user_id', $this->oauth_user_id, TRUE))
		{
			$DPFields = array(
			...
			$CCDetails = array(
				'creditcardtype'	=> $this->input->post('credit_card_type'),	// Required. Type of credit card.  Visa, MasterCard, Discover, Amex, Maestro, Solo.  If Maestro or Solo, the currency code must be GBP.  In addition, either start date or issue number must be specified.
				'acct' 				=> $this->input->post('credit_card'),      	// Required. Credit card number.  No spaces or punctuation.  
				'expdate' 			=> $this->input->post('expire_date'),		// Required. Credit card expiration date.  Format is MMYYYY
				'cvv2' 				=> $this->input->post('cvv2'),      		// Requirements determined by your PayPal account settings.  Security digits for credit card.
				'startdate' 		=> $this->input->post('start_date'),        // Month and year that Maestro or Solo card was issued.  MMYYYY
				'issuenumber' 		=> $this->input->post('issue_number')       // Issue number of Maestro or Solo card.  Two numeric digits max.
			);
			...
			$PaymentDetails = array(
				'amt' 				=> $this->input->post('order_total'),   // Required. Total amount of order, including shipping, handling, and tax.  
				...
				'shipdiscamt' 		=> $this->input->post('shipdiscamt'),   // Shipping discount for the order, specified as a negative number.  
				'handlingamt' 		=> $this->input->post('handlingamt'),   // Total handling costs for the order. If you specify handlingamt, you must also specify itemamt.
				'taxamt' 			=> $this->input->post('taxamt'),        // Required if you specify itemized cart tax details. Sum of tax for all items on the order. Total sales tax. 
			...
			$Item = array(
				'l_name' 			=> $this->input->post('order_name'),        // Item Name.  127 char max.
				'l_desc' 			=> $this->input->post('order_description'), // Item description.  127 char max.
				'l_amt' 			=> $this->input->post('order_total'),       // Cost of individual item.
				'l_number' 			=> $this->input->post('order_id'),          // Item Number.  127 char max.
		    ...
		    $paypal_result = $this->paypal_pro->DoDirectPayment($paypal_request_data);
	
		    if (!$this->paypal_pro->APICallSuccessful($paypal_result['ACK']))
			{
	            $message = array('status' => 'error', 'message' => 'We could not accept your payment', 'data' => $paypal_result['ERRORS']);
	        }
	        else
	        {
	            $message = array('status' => 'success', 'message' => 'Your payment was received', 'data' => $paypal_result);        
	        }	
        }
        else
        {
            $message = array('status' => 'error', 'message' => 'Oops, there was an error paying with that user account!');	        
        }	
